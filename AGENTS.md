...
│   │   ├── quota.go              #   FetchQuota — `claude -p /usage` → the subscription limits the quota row shows
...
│   │   ├── quota.go                      #   quota row under the header: the once-a-minute /usage poller and its rendering
...
- **Usage gauges.** A dedicated row under the header shows the Claude subscription limits in every view, including the loading screen: the five-hour window and the all-models week, each as a gauge, a percentage that turns amber at 60% and red over 80%, and when it resets — relative while that's under a day out ("resets in 42m"), a weekday and time once it isn't ("resets Thu 11pm") (`quota.go`). A goroutine beside the session poller runs `agent.FetchQuota` (`claude -p --output-format json --no-session-persistence --strict-mcp-config /usage`, a local command that reaches no model) at startup and every minute. A failed call keeps the last reading, faint; an API-key login (no subscription limits) or a missing `claude` shows nothing. On a narrow terminal the row sheds the bars, then shortens and drops the reset times, and only then truncates — the percentages themselves are never silently dropped.
