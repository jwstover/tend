...
- **Launch/resume.** `internal/agent.LaunchCmd`/`ResumeCmd` build the `claude` invocation; the TUI hands it to `tea.ExecProcess` for the terminal handoff. A session's `claude --session-id` is generated up front (`session_id.go`) so tend never has to discover it after the fact. `LaunchCmdWith` adds the per-step extras a workflow run needs — an initial prompt (claude's positional argument), `--model`, `--permission-mode` and `--append-system-prompt` — and is otherwise `LaunchCmd`; `ResumeCmdWith` is `ResumeCmd` plus the system prompt block.
