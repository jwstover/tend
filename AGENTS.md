...
│   │   ├── quota.go              #   FetchQuota — `claude -p /usage` → the subscription limits the header gauges show
...
│   │   ├── quota.go                      #   header usage gauges: the once-a-minute /usage poller and its rendering
...
- **Usage gauges.** The header's far right shows the Claude subscription limits in every view: the five-hour window and the all-models week, each as a gauge and a percentage that turns amber at 60% and red over 80% (`quota.go`). A goroutine beside the session poller runs `agent.FetchQuota` (`claude -p --output-format json --no-session-persistence --strict-mcp-config /usage`, a local command that reaches no model) at startup and every minute. A failed call keeps the last reading, faint; an API-key login (no subscription limits) or a missing `claude` shows nothing. On a narrow terminal the bars go first, then the view's own right-hand text, then the gauges.
