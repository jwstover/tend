...
- **Glyphs.** `TEND_GLYPHS` picks the symbol set in `styles.go`: unset or `unicode` (the default), `nerd` for a terminal with a Nerd Font (the usage gauges become Fira Code's joined progress bar with hourglass/calendar labels), `ascii` for a terminal without usable Unicode.
