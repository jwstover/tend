...
│   │   └── brief.go          #   SessionBrief + SessionSystemPrompt — the task-context block an interactive session's system prompt carries
...
│   │   ├── tmux.go               #   dedicated-socket tmux wrapping: wrap/attach/has-session/kill/capture-pane; CommandFitsTmux (the 16KB client→server cap)
│   │   ├── hooks.go              #   Claude Code hook payload parsing + injected --settings generation
│   │   ├── quota.go              #   FetchQuota — `claude -p /usage` → the subscription limits the header's quota segment shows
...
│   │   ├── system_prompt.go       #   WriteSystemPrompt — per-session --append-system-prompt-file for the task brief, which inline would blow tmux's 16KB command cap
...
│   │   ├── launch.go               #   Launch/Resume/Retry: host a runner in a detached tmux session, refuse a live or ended run; Retry reopens a failed one
...
│   │   ├── runs.go                  #   run inspection tools (list_workflow_runs, get_workflow_run, get_step_run_usage), registered for every session, read-only
...
│   │   ├── picker.go / fuzzy.go              #   the shared fuzzy chooser overlay every picker is built from (picker[T]) and its fuzzyFilter
│   │   ├── palette.go / urlpicker.go / whichkey.go / modal.go / help.go   #   supporting overlays
│   │   ├── quota.go                      #   header's centered quota segment: the once-a-minute /usage poller and its rendering
...
CREATE TABLE workflow_steps (       -- a node: kind 'agent' (headless claude driven by prompt_md) or 'gate' (a pause for a
  ...
  model, permission_mode, advisor_model              -- '' = inherit claude's default; opaque to tend, forwarded as-is
    TEXT NOT NULL DEFAULT '',                        -- (advisor_model is claude's --advisor, 00017)
...
CREATE TABLE workflow_runs (        -- one execution of a workflow against a task; cascades with the task (like a session)
  ...
  current_step_run_id INTEGER                    -- through, so the schema pins the set. Terminal is final, except failed -> paused (RetryRun).
...
CREATE TABLE workflow_step_runs (   -- one execution of one step within a run: one claude session for an agent step, a
  ...
  prompt_rendered, model, permission_mode,           -- what ACTUALLY ran (the definition is read live)
    advisor_model TEXT NOT NULL DEFAULT '',
...
- **Workflows** are global definitions; a **run** binds one to a task, and a **step run** is one execution of one step inside a run (one claude session for an agent step). Execution order comes from `workflow_edges`, never `sort_order`: the runner picks every step after the first by looking up the previous step run's outcome among the edges leaving its step. The Store owns the transitions (`ClaimRun` is a compare-and-swap from `pending` or `paused`, `SetRunState` refuses a terminal run with `workflow.ErrRunEnded`, `RetryRun` is the one way out of a terminal state — `failed` back to `paused`, refusing anything else with `workflow.ErrRunNotFailed` — and `FinishStepRun` is one-shot and refuses an outcome the step's edges don't route). `workflow_runs.error` describes the state a run is in, not its history: `FailRun` writes it, `RetryRun` keeps it as the reason the run is paused, and `ClaimRun`/`SetRunState` clear it. See §8, "Workflows".
...
| `tend workflow approve <run-id> [--feedback ...]` / `reject <run-id> --feedback ...` / `decide <run-id> <outcome> [--feedback ...]` | Decide the gate a run is waiting at: the same one-shot `FinishStepRun` the TUI's `a`/`A`/`x` and `finish_step` make. `--feedback` is the gate's deliverable — a message for the next step — and `runner.nextStep` hands it on as that step's `{{.Feedback}}` either way: on a loop-back edge (reject) the target keeps its input and reworks against the text; on a forward edge (approve with a message, tend task #322) the text rides *alongside* the reviewed deliverable, which stays the next step's `{{.Input}}`. So `reject` requires non-blank feedback, and `approve` leaves it out by default so the gate passes its input through alone, as the TUI's `a` does. `decide <run-id> <outcome>` is the same write for a gate whose edges route other outcomes (a "Needs human" gate routing `continue`/`stop`, the TUI's `o` picker): the outcome is normalized as edges are stored (trimmed, lower-cased), and `status <run-id>` hints `decide` per routed outcome for such a gate. Refused, naming why, for a run not waiting for review, a current step that is not a gate, a gate already decided, or an outcome the gate's edges do not route. |
...
| `tend workflow retry <run-id> [--fresh]` | Re-enter a failed run at the step that failed (`runner.Retry`): `RetryRun` takes it back to `paused`, keeping the failure on the row, and a runner is launched with `--takeover` as `resume` does. Without `--fresh` the failed step's claude session is continued with `workflow.RetryPrompt` quoting the failure; with it the step starts over on a new session (`runner.RestartStep`). Same step run and iteration either way, and the step runs with the model, permission mode and advisor model its step has now, so fixing the step first is enough. `status <run-id>` prints both forms under a failed run. Refuses a run that has not failed (`workflow.ErrRunNotFailed`; a paused run is `resume`d) or whose runner is alive. |
...
- **List view (default).** A grouped/tree view of the live view (sub-tasks nest under their parent), with vim-style navigation, search, a `:`/`Ctrl-P` command palette, and quick add. Tasks group by state by default; the `g` chord regroups by priority or by latest agent-session status (`gg` stays "top of list"). A project column scopes the list to the selected project (or shows all); `A` in the column archives the selected project, which drops out of the column but keeps its tasks, and `C` there shows the archived projects as a marked, dimmed shelf under the active ones so `A` can restore one (the `P` picker never offers an archived project as a destination). Archiving and `dd` deleting a project both stop at a which-key panel that names the project (and, for a delete, how many live tasks move to Unsorted) and go ahead only on `y`; restoring is immediate, being the undo. `dd` on a task (and the palette's delete) stops the same way, naming the task and how many sub-tasks cascade with it (`confirm.go`). `m` opens a move-to-parent picker (fuzzy-filtered over the breadcrumb paths) that re-parents the selected task and its sub-tree under another task in the project, or promotes it to the top level (the `(top level)` row). `b` opens the **dependency picker** (`dependencypicker.go`, fuzzy over title and project): every open task across every project, the ones the selected task already waits on checked and listed first; ⏎ or a digit toggles a row (`AddDependency`/`RemoveDependency`) and the picker stays open, so several blockers are set in one visit; a refused edge (a cycle) shows in the status line and leaves the row unchecked. The list row carries a dependency cell after the sub-task count: `⊘N` in the blocked colour while N blockers are still open, a green check once every blocker is done (the task is free to start), blank when it waits on nothing.
...
- **Launch/resume.** `internal/agent.LaunchCmd`/`ResumeCmd` build the `claude` invocation; the TUI hands it to `tea.ExecProcess` for the terminal handoff. A session's `claude --session-id` is generated up front (`session_id.go`) so tend never has to discover it after the fact. `LaunchCmdWith` adds the per-step extras a workflow run needs — an initial prompt (claude's positional argument), `--model`, `--permission-mode`, `--advisor` and `--append-system-prompt` — and is otherwise `LaunchCmd`; `ResumeCmdWith` is `ResumeCmd` plus the system prompt block and `--advisor` (unlike model and permission mode, claude re-reads the advisor on every invocation, so a resume forwards it too; verified against claude 2.1.272, tend task #32).
- **Task brief (tend task #292).** Every interactive launch, and every fresh `claude --resume` (not a tmux re-attach, which starts no process), gets the bound task as `--append-system-prompt-file`: `task.SessionSystemPrompt` renders a `task.SessionBrief` — the task's facts (state, project and its cwd, priority, due, tags, dates), its parent, its sub-tasks with their states and a "waiting on N" marker, the `BLOCKED BY` and `BLOCKS` sets, the body, and the ten most recent log entries — with a preamble saying the session is bound to the task, that the block is a launch-time snapshot, and that the tend MCP tools read and change the live task. The TUI assembles the brief from the store inside the launch/resume Cmd (`sessionBriefPrompt` in `sessions.go`), so a session no longer has to open with "review the current tend task". Like the MCP config, it is a convenience layered on the launch, not a condition of it: an unreadable task yields no block, and a relation that fails to load just leaves its section out. Headless workflow steps are unchanged — their prompt template already carries the task (`{{.Task.Body}}`, `{{.Subtasks}}`).
  - *Why a file, and the three guardrails.* tmux ships the whole `new-session -- claude …` argv from client to server in one 16KB imsg and exits 1 with "command too long" past that, before claude ever runs — so with the brief inline as `--append-system-prompt`, every launch on a task whose body had grown past ~15KB died on the spot (eight tasks in a real database were between 54KB and 270KB). (1) The brief now goes through `agent.WriteSystemPrompt`, a temp file with the MCP config's lifetime (removed when the session really ends, not when it is backgrounded), so the argv carries a path. (2) `task.SessionSystemPrompt` caps the body at `briefBodyLimit` (48KB, ~12k tokens; the brief is sent on every request): a longer body is abridged to its head (the original description) and tail (the latest appended notes) around an "omitted here" marker, on line boundaries, and the heading says how much was kept and that `mcp__tend__get_current_task` has the rest. (3) `wrapInTmux` checks `agent.CommandFitsTmux` and runs claude directly, without backgrounding, if a wrapped command would ever exceed the cap again. The inline `LaunchOpts.AppendSystemPrompt` stays for the runner's headless steps, which exec claude directly (1MB ARG_MAX, not 16KB).
...
- **Authoring over MCP.** `internal/mcpserver/workflows.go` exposes enough of the store's workflow, step and edge writes for an agent session to draft a workflow from a description (tend task #188): `list_workflows`, `get_workflow` (by id or case-insensitive name), `create_workflow`, `update_workflow` (rename/description), `add_workflow_step`, `update_workflow_step` (name, kind, model, permission mode, advisor model), `set_step_prompt`, `reorder_workflow_steps`, `delete_workflow_step`, `add_workflow_edge` and `delete_workflow_edge`. They are registered for every session (`registerWorkflowTools` in `server.go`), not only a step's — nothing here is bound to a task or a run. The convention is that `get_workflow` and **every mutation** return the whole graph: the steps in authoring order with their prompts, settings and outgoing edges (`to_step` by id and name, `max_iterations`), a `preview` from `workflow.PreviewText` (the same text the Workflows view shows) and `problems` from `workflow.Validate` — so the agent sees the effect of each edit, and what the validator would flag, without a second call; a workflow with no steps yet is a draft, not a problem, and reports none. `add_workflow_step` mirrors the TUI's default-edge convenience: unless `link_from_previous` is false, the previous step, if it has no edges yet, is linked to the new one with `done -> new`, so steps added in order form a linear workflow with no edge work (the previous step is read before the append, so "previous" is unambiguous when two sessions author at once). `prompt_md` goes through `workflow.ValidatePrompt` on `add_workflow_step` and `set_step_prompt` — a template that does not render is refused at authoring time rather than failing every run at launch (a gate's prompt is ignored). `model`, `permission_mode` and `advisor_model` accept `inherit` for the stored `""`; permission modes are restricted to the TUI picker's four (`default`, `acceptEdits`, `bypassPermissions`, `plan`) because a value claude rejects fails the run long after authoring, while models and advisor models are not restricted to their picker aliases since claude also accepts full ids (claude itself validates a pairing it cannot support — an advisor weaker than the main model, say — at launch, so tend does not duplicate that table). `add_workflow_edge` upserts on `(from_step_id, outcome)` like `SetEdge`, and `delete_workflow_edge` addresses an edge by the same natural key — "drop review's reject route" — rather than an edge id the agent would have to look up first; a miss names the outcomes the step does route. `delete_workflow_step` removes the step and every edge touching it, refused once a live run has executed it. There is deliberately **no `delete_workflow`**: an agent confusing two ids should at worst lose a step it added, not a procedure the user authored; deleting a workflow stays in the TUI. These tools are for drafting — the Workflows view (§7), which reloads live as the rows land, is where the user refines the result.
- **Inspecting a run over MCP.** `internal/mcpserver/runs.go` (`registerRunTools`, every session, tend task #327) is the read-only counterpart to authoring: `list_workflow_runs` (defaulting to the bound task) lists a task's runs newest first with each one's current step; `get_workflow_run` returns one run's step runs in order plus a per-step total (how many times each step ran, for how long) — the prompt, input, feedback and deliverable are reported by length only, `include_deliverables`/`include_inputs` opt in to the text itself, so a long-running loop does not flood the context by default; `get_step_run_usage` tallies one step run's stream-json log (`internal/usage.ParseStepLog`) into cost, tokens, turns, duration, permission denials, sub-agent stats and tool-call counts (top-level and sub-agent, with the bytes each tool's results put back into context) — counters summed across a resumed step's repeated result records, cost and the per-model breakdown taken from the last one rather than summed. Before these, answering "why did that run take four passes" or "where did the tokens go" needed direct SQLite and log-file access.
