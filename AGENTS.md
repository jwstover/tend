...
**`Tend.Template` is checked against Go, not against itself.** The port renders stored `prompt_md`, so `elixir/test/support/parity*` renders every template in the repo's Go tests — and, on demand, every `prompt_md` row in a real `tend.db` — through *both* Go's `text/template` and `Tend.Template`, and asserts byte equality. `mix test` compares against the Go output recorded in `elixir/test/fixtures/parity/repo_prompts.jsonl`; where a Go toolchain is on the `PATH` it additionally re-checks that recording against live Go. The whole thing, including the `tend.db` half, is `MIX_ENV=test mix tend.parity --db PATH`; `--update` re-records after a Go test adds or changes a template. **Without Go the suite still goes green but has not checked parity**, so it prints a banner saying exactly that — do not read a Go-less green run as a passing parity check. The banner reports what the run *executed*, not what the machine could have: `mix test --exclude go`, `--only some_other_tag` and any single-file `mix test` all print `NOT A PARITY RUN` on a machine that has Go, because none of them ran the `:go`-tagged comparison.
