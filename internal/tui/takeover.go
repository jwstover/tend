...
var takeoverResume = func(sess task.Session, dbPath, systemPrompt, advisorModel string, ref takeoverRef) tea.Cmd {
	return resumeSessionCmd(sess, dbPath, systemPrompt, advisorModel, ref)
}
	...
	return func() tea.Msg {
		...
		for _, sess := range sessions {
			if sess.ExternalID == cur.SessionExternalID {
				return takeoverResume(sess, a.dbPath, a.sessionBriefPrompt(sess.TaskID), cur.AdvisorModel, takeoverRef{runID: run.ID, stepRunID: cur.ID})()
			}
		}
